...
def remove_file(file_path):
    call(["rm", "-rf", file_path, ], encoding='utf8')
...
def get_dependency_list(key, unzip_dir_path, copied_jar_paths):
    ...
    for pom in all_poms:
        if exists(pom):
            try:
                jat_list_path = copied_jar_paths + "/build_classpath.txt"
                output = check_output(
                    ["mvn", "dependency:build-classpath",
                     "-Dmdep.outputFile=" + jat_list_path, "-f", pom],
                    encoding='utf8')
                jars.extend(list(read_and_remove_jar_list(jat_list_path)))
            except Exception as e:
...
def find_dependency_collection_in_repo(zip_paths, unzip_path, base_path, path_copied_jars, threadid):
    ...
    jar_list_path_for_thread = path_copied_jars + "/" + "dir_" + str(threadid)
    clean_unzip_dir(unzip_folder_for_thread)
    clean_unzip_dir(jar_list_path_for_thread)
    ...
    for path in zip_paths:
        try:
            if unzip_repository(path, unzip_folder_for_thread):
                ...
                repo = get_dependency_jars_names(key, path, unzip_folder_for_thread, jar_list_path_for_thread)
...
if __name__ == '__main__':
    ...
    # mvn dependency:build-classpath -Dmdep.outputFile=build_classpath.txt
