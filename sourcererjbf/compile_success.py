#!/usr/bin/env python
import json, sys, os
pbs = dict((os.path.join(f1, f2), json.load(open(os.path.join(sys.argv[1], f1, f2, "build-result.json")))) for f1 in os.listdir(sys.argv[1]) for f2 in os.listdir(os.path.join(sys.argv[1], f1)))
...
def get_details(pid, details):
  ...
  try:
    time_of_last_build = details["timing"][build_start_index[-1] + 1][1] - details["timing"][build_start_index[-1]][1]
  except Exception:
    time_of_last_build = -1
  number_of_unique_errors = len(set(err["error_type"] for err in details["output"])) if type(details["output"]) == list else 0
...
with open(sys.argv[2], "w") as wf:
  wf.write("pid,success,was_android,had_encoding_fixed,had_dependencies_fixed,our_build_file_used,has_own_build,total_time_spent,number_of_build_tries,time_of_last_build,unique_err_count,used_ant,used_mvn,used_gradle\n")
  wf.write("\n".join(detailed_info) + "\n")
